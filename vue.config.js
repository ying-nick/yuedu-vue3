...
module.exports = {
  ...
  devServer: {
    ...
    proxy: {
      ...
      //阅小说
      "/ynv/*": {
        target: "http://yuenov.com:18888/", // 目标服务器地址
        secure: false, // 目标服务器地址是否是安全协议
        changeOrigin: true, // 是否修改来源, 为true时会让目标服务器以为是webpack-dev-server发出的请求!服务端和服务端的请求是没有跨域的
        pathRewrite: { "^/ynv": "/" } // 将/api开头的请求地址, /api 改为 /, 即 /api/xx 改为 /xx
      },
      //阅小说图片
      "/ypic/*": {
        target: "http://pt.yuenov.com:18888/", // 目标服务器地址
        secure: false, // 目标服务器地址是否是安全协议
        changeOrigin: true, // 是否修改来源, 为true时会让目标服务器以为是webpack-dev-server发出的请求!服务端和服务端的请求是没有跨域的
        pathRewrite: { "^/ypic": "/" } // 将/api开头的请求地址, /api 改为 /, 即 /api/xx 改为 /xx
      },
      //追书图片
      "/pic/*": {
